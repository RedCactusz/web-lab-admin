...
import { superAdminService, type NewsData } from "@/app/services/superAdminService";

export default function KelolaNewsPage() {
  const [data, setData] = useState<NewsData[]>([]);
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<NewsData | null>(null);
  const [deleteItem, setDeleteItem] = useState<NewsData | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editingItem) {
      await superAdminService.news.update(editingItem.id, form);
    } else {
      await superAdminService.news.create(form);
    }
    closeModal();
    loadData();
  };
  ...
  const handleDelete = async () => {
    if (deleteItem) {
      await superAdminService.news.delete(deleteItem.id);
      setDeleteItem(null);
      loadData();
    }
  };
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 text-sm">Manajemen konten berita laboratorium</p>
        </div>
        <button
          onClick={() => {
            setEditingItem(null);
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                <tr><td colSpan={5} className="text-center py-12 text-gray-400 italic">Belum ada data berita</td></tr>
              ) : (
                filtered.map((item) => (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 rounded-t-2xl">
              <h2 className="text-lg font-bold text-gray-900">{editingItem ? "Edit Berita" : "Tambah Berita"}</h2>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
                </div>
              </div>
              <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="text-sm font-semibold text-gray-700">Terbitkan Berita</label>
              </div>
