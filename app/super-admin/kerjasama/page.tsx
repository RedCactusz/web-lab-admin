...
export default function KelolaKerjasamaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        const result = await superAdminService.kerjasama.update(editingItem.id, form);
        if (result) {
          toast.success("Kerjasama berhasil diperbarui!");
        } else {
          toast.error("Gagal memperbarui kerjasama");
          return;
        }
      } else {
        const result = await superAdminService.kerjasama.create(form);
        if (result) {
          toast.success("Kerjasama berhasil ditambahkan!");
        } else {
          toast.error("Gagal menambahkan kerjasama");
          return;
        }
      }
      closeModal();
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-all shadow-lg shadow-indigo-900/20 disabled:opacity-50 disabled:cursor-not-allowed">{loading ? "Memproses..." : (editingItem ? "Simpan" : "Tambah")}</button>
              </div>
            </form>
          </div>
        </div>
